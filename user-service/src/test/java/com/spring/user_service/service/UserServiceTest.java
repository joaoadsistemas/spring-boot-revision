...
class UserServiceTest {
    ...
    void findAll_shouldReturnAllElements_whenSuccessfully() {
        BDDMockito.when(userRepository.findAll()).thenReturn(userSet);
    ...
    void findById_shouldReturnUser_whenSuccessfully() {
        ...
        BDDMockito.when(userRepository.findById(ArgumentMatchers.anyLong())).thenReturn(expectedResult);
    ...
    void findByEmail_shouldReturnUser_whenSuccessfully() {
        var expectedResult = userSet.stream().filter(u -> u.getId().equals(1L)).findFirst().get();
        BDDMockito.when(userRepository.findByEmail(ArgumentMatchers.anyString())).thenReturn(expectedResult);
    ...
    void save_shouldSaveUser_whenSuccessfully() {
        ...
        BDDMockito.doNothing().when(userRepository).save(ArgumentMatchers.any(User.class));
    ...
    void delete_shouldDeleteUser_whenSuccessfully() {
        ...
        BDDMockito.doNothing().when(userRepository).delete(ArgumentMatchers.anyLong());
    ...
    void delete_shouldThrowAnException_whenIdDoesNotExists() {
        BDDMockito.doThrow(ResponseStatusException.class).when(userRepository).delete(ArgumentMatchers.anyLong());
        var id = 99L;
    ...
    void put_shouldReturnObject_whenSuccessfully() {
        ...
        BDDMockito.when(userRepository.findById(userBeforeModification.getId())).thenReturn(userBeforeModification);
        ...
        BDDMockito.when(userRepository.update(ArgumentMatchers.any(User.class))).thenReturn(userAfterModification);
    ...
    void put_shouldThrowAnException_whenIdDoesNotExists() {
        ...
        BDDMockito.when(userRepository.update(ArgumentMatchers.any(User.class))).thenThrow(ResponseStatusException.class);
